...
3) JSX 변수는 {so}와 같이 중괄호 사이에 넣어 읽을 수 있습니다.
4) JSX 속성은 HTML 속성과 다르기 때문에 JavaScript의 예약어와 충돌하지 않습니다. 예를 들어 HTML의 class는 JSX에서는 className입니다. 여러 개의 단어로 된 속성을 camelCase로 명명한다는 것을 기억해주세요.
5) props는 컴포넌트 호출에 속성처럼 쓰이고, 컴포넌트로 전달됩니다.
